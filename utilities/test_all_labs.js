...
totalCommands++; try { var result54 = db.policies.aggregate([{$group: {_id: "$policyType", count: {$sum: 1}}}, {$sort: {count: -1}}]); print("✅ Count policies by type"); passedCommands++; } catch (error) { print("❌ Count policies by type - ERROR: " + error.message); errors.push("Lab7 policyCount: " + error.message); failedCommands++; }
totalCommands++; try { var result55 = db.policies.aggregate([{$match: {isActive: true}}, {$group: {_id: "$agentId", totalRevenue: {$sum: "$annualPremium"}, policyCount: {$sum: 1}}}, {$sort: {totalRevenue: -1}}]); print("✅ Revenue analysis by agent"); passedCommands++; } catch (error) { print("❌ Revenue analysis by agent - ERROR: " + error.message); errors.push("Lab7 agentRevenue: " + error.message); failedCommands++; }
totalCommands++; try { var result56 = db.claims.aggregate([{$match: {claimStatus: "approved"}}, {$group: {_id: {year: {$year: "$filedDate"}, month: {$month: "$filedDate"}}, averageClaimAmount: {$avg: "$claimAmount"}, totalClaims: {$sum: 1}}}, {$sort: {"_id.year": 1, "_id.month": 1}}]); print("✅ Claims analysis by month"); passedCommands++; } catch (error) { print("❌ Claims analysis by month - ERROR: " + error.message); errors.push("Lab7 claimsMonth: " + error.message); failedCommands++; }
totalCommands++; try { var result57 = db.policies.aggregate([{$lookup: {from: "customers", localField: "customerId", foreignField: "_id", as: "customerInfo"}}, {$unwind: "$customerInfo"}, {$project: {policyNumber: 1, policyType: 1, annualPremium: 1, "customerInfo.customerName": 1, "customerInfo.email": 1}}]); print("✅ Join policies with customers"); passedCommands++; } catch (error) { print("❌ Join policies with customers - ERROR: " + error.message); errors.push("Lab7 policyCustomers: " + error.message); failedCommands++; }
totalCommands++; try { var result58 = db.policies.aggregate([{$unwind: "$coverageTypes"}, {$group: {_id: "$coverageTypes", count: {$sum: 1}, averagePremium: {$avg: "$annualPremium"}}}, {$sort: {count: -1}}]); print("✅ Coverage types analysis"); passedCommands++; } catch (error) { print("❌ Coverage types analysis - ERROR: " + error.message); errors.push("Lab7 coverageTypes: " + error.message); failedCommands++; }
totalCommands++; try { var result59 = db.policies.aggregate([{$match: {policyType: "AUTO"}}, {$count: "autoCount"}]); print("✅ Auto policy verification"); passedCommands++; } catch (error) { print("❌ Auto policy verification - ERROR: " + error.message); errors.push("Lab7 autoVerify: " + error.message); failedCommands++; }
