...
totalCommands++; try { db.reviews.createIndex({reviewText: "text", categories: "text"}); print("✅ Create text index"); passedCommands++; } catch (error) { print("❌ Create text index - ERROR: " + error.message); errors.push("Lab6 textIndex: " + error.message); failedCommands++; }
totalCommands++; try { var result49 = db.reviews.find({$text: {$search: "service excellent customer"}}); print("✅ Text search query"); passedCommands++; } catch (error) { print("❌ Text search query - ERROR: " + error.message); errors.push("Lab6 textSearch: " + error.message); failedCommands++; }
totalCommands++; try { var result50 = db.agents.find({$and: [{"performance.customerRating": {$gte: 4.5}}, {"performance.salesActual": {$gt: 500000}}]}); print("✅ Agent performance query"); passedCommands++; } catch (error) { print("❌ Agent performance query - ERROR: " + error.message); errors.push("Lab6 agentPerf: " + error.message); failedCommands++; }
totalCommands++; try { var result51 = db.customers.find({$or: [{"address.state": "NY"}, {"address.state": "CA"}]}); print("✅ Nested field query"); passedCommands++; } catch (error) { print("❌ Nested field query - ERROR: " + error.message); errors.push("Lab6 nested: " + error.message); failedCommands++; }
