...
totalCommands++; try { var oneYearFromNow = new Date(); oneYearFromNow.setFullYear(oneYearFromNow.getFullYear() + 1); var result47 = db.policies.find({expirationDate: {$gte: new Date(), $lte: oneYearFromNow}}); print("✅ Date range query"); passedCommands++; } catch (error) { print("❌ Date range query - ERROR: " + error.message); errors.push("Lab6 dateRange: " + error.message); failedCommands++; }
